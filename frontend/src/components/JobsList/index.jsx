...
const JobsList = () => {
  ...
  const fetchJobs = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${API_URL}?page=${currentPage}&limit=9&search=${search}`);
  ...
  const handleDelete = async (jobId) => {
    try {
      ...
      await axios.delete(`${API_URL}/${jobId}`, config);
      fetchJobs();
    } catch {
  ...
  return (
    <div className="p-8 min-h-screen" style={{ backgroundColor: "#FFFDF2", color: "#000000" }}>
      {/* Header */}
...
<div className="flex justify-center mt-8 space-x-4">
  <button
    ...
    style={{
      backgroundColor: currentPage === 1 ? "#CCC" : "#000000",
      color: "#FFFDF2",
      border: "2px solid #000000",
    }}
  >
  ...
  <button
    ...
    className="px-4 py-2 rounded-md font-semibold transition-colors duration-300 disabled:opacity-50"
    style={{
      backgroundColor: currentPage === totalPages ? "#CCC" : "#000000",
      color: "#FFFDF2",
      border: "2px solid #000000",
    }}
  >
